  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Link href={backHref} className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-charcoal/60 hover:text-charcoal">
            <ChevronLeft className="w-4 h-4" /> Back to list
          </Link>
        }
      />
      <div className="bg-white border border-charcoal/10 rounded-lg p-6">
        <SchemaForm fields={fields} initial={initial} onSubmit={handleSubmit} submitLabel={isNew ? "Create" : "Save changes"} />
      </div>
    </div>
  );
}
