...
/* ------------------------------------------------------------------ */
/* Animations toggle — only disabled for users who prefer reduced     */
/* motion. Previously also disabled on mobile, which hid all section   */
/* entrances — now every section animates on every viewport.           */
/* ------------------------------------------------------------------ */
const MOBILE_QUERY = "(prefers-reduced-motion: reduce)";
...
/* Each child animates on its OWN whileInView so items can never stay  */
    ...
    <div className={className}>
      {Children.map(children, (child, i) =>
        isValidElement(child)
          ? cloneElement(
              child as React.ReactElement<{ staggerDelay?: number }>,
              { staggerDelay: (child.props as { staggerDelay?: number }).staggerDelay ?? i * gap }
            )
          : child
      )}
    </div>
...
export function RevealItem({
  ...
  const hiddenMap: Record<string, Record<string, number>> = {
...
export function TiltCard({
  ...
  const glare = useMotionTemplate`radial-gradient(420px circle at ${gx}% ${gy}%, rgba(221,161,38,0.18), transparent 60%)`;
...
/* ------------------------------------------------------------------ */
/* AnimatedIcon — micro-interaction for lucide icons                   */
/* ------------------------------------------------------------------ */
